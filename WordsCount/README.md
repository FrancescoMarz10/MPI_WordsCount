...
All'interno del package, per agevolare i test è stato inserito uno script, fileGenerator1.java, ed un dizionario word.txt, per generare file con parole casuali della dimensione inserita all'interno del programma. Tale script crea un file denominato 'fileGenerato.txt'. Dunque, nel caso in cui si desideri generare più file, è necessario rinominare qulello appena creato ed aggiornare i file list corrispondenti. Una volta terminata tale procedura sul master, la cartella Files può essere inviata agli slaves.
